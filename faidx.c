...
static inline void
fai_insert_index(faidx_t *idx, const char *name, int len, int line_len, int line_blen, unsigned long long offset)
{
    int ret = 0;
    khint_t k;
    faidx1_t t;
...
void
fai_save(const faidx_t *fai, FILE *fp)
{
    int i = 0;
    khint_t k;
    ...
    for (i = 0; i < fai->n; ++i)
        {
            ...
            fprintf(fp, "%s\t%d\t%lld\t%d\t%d\n", fai->name[i], (int)x.len, 
                   (long long)x.offset, (int)x.line_blen, (int)x.line_len);
        }
}
...
char *
fai_fetch(const faidx_t *fai, const char *str, int *len)
{
    char *s;
    char c;
    int i;
...
int
faidx_fetch_nseq(const faidx_t *fai)
{
    return fai->n;
}

char *
faidx_fetch_seq(const faidx_t *fai, char *c_name, int p_beg_i, int p_end_i, int *len)
{
    int l;
    char c;
    char *seq = NULL;
