...
static inline lbnode_t *
mp_alloc(mempool_t *mp)
{
    ++mp->cnt;
    if (mp->n == 0)
        {
...
static inline void
mp_free(mempool_t *mp, lbnode_t *p)
{
    --mp->cnt;
...
bam_plp_t
bam_plp_init(bam_plp_auto_f func, void *data)
{
    bam_plp_t iter;
...
void
bam_plp_destroy(bam_plp_t iter)
{
...
const bam_pileup1_t *
bam_plp_next(bam_plp_t iter, int *_tid, int *_pos, int *_n_plp)
{
    if (iter->error)
        {
            *_n_plp = -1;
...
int
bam_plp_push(bam_plp_t iter, const bam1_t *b)
{
    if (iter->error)
        {
            return -1;
...
bam_plbuf_t *
bam_plbuf_init(bam_pileup_f func, void *data)
{
    bam_plbuf_t *buf;
...
void
bam_plbuf_destroy(bam_plbuf_t *buf)
{
...
int
bam_plbuf_push(const bam1_t *b, bam_plbuf_t *buf)
{
    int ret = 0;
    int n_plp = 0;
    int tid = 0;
