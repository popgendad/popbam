...
const char *o_rg_tags[]      = {"CN", "DS", "DT", "FO", "KS", "LB", "PG", "PI", "PL", "PU", NULL};
const char *r_rg_tags[]      = {"ID", "SM", "PO", NULL};
...
static list_t *
list_append_to_end(list_t *root, void *data)
{
...
static list_t *
list_append(list_t *root, void *data)
{
    list_t *l = root;
...
static int
tag_exists(const char *tag, const char **tags)
{
    int itag = 0;
...
static HeaderTag *
new_tag(const char *name, const char *value_from, const char *value_to)
{
    HeaderTag *tag = (HeaderTag*)malloc(sizeof(HeaderTag));
    int len = value_to - value_from + 1;

    tag->key[0] = name[0];
    tag->key[1] = name[1];
    tag->value = (char*)malloc(len+1);
...
static HeaderTag *
header_line_has_tag(HeaderLine *hline, const char *key)
{
...
static int
sam_header_compare_lines(HeaderLine *hline1, HeaderLine *hline2)
{
    int itype = 0;
    int itag = 0;
    int missing = 0;
    ...
    if (!required_tags[itype] && !optional_tags[itype])
        {
            t1 = (HeaderTag*)hline1->tags->data;
            t2 = (HeaderTag*)hline2->tags->data;
            if (!strcmp(t1->value, t2->value))
                {
                    return 1;    // identical comments
                }
            return 0;
        }
    missing = 0;
    ...
    if (missing)
        {
            return 3;    // there are some missing complementary tags with no conflicts, can be merged
        }
    return 1;
}

static HeaderLine *
sam_header_line_clone(const HeaderLine *hline)
{
    list_t *tags;
    HeaderLine *out = (HeaderLine*)malloc(sizeof(HeaderLine));
    HeaderTag *old;
    HeaderTag *nw;
    ...
    while (tags)
        {
            old = (HeaderTag*)tags->data;
            nw = (HeaderTag*)malloc(sizeof(HeaderTag));
            nw->key[0] = old->key[0];
            nw->key[1] = old->key[1];
            nw->value  = strdup(old->value);
            out->tags = list_append(out->tags, nw);
            tags = tags->next;
            free(nw);
        }
    return out;
...
static int
sam_header_line_merge_with(HeaderLine *out_hline, const HeaderLine *tmpl_hline)
{
    list_t *tmpl_tags;

    if ((out_hline->type[0] != tmpl_hline->type[0]) ||
            (out_hline->type[1] != tmpl_hline->type[1]))
        {
            return 0;
        }
    ...
    while (tmpl_tags)
        {
            HeaderTag *tmpl_tag = (HeaderTag*)tmpl_tags->data;
...
static int
sam_header_line_validate(HeaderLine *hline)
{
    int itype = 0;
    int itag = 0;
    list_t *tags;
            ...
            if (!header_line_has_tag(hline,required_tags[itype][itag]))
                {
                    debug("The tag [%c%c] required for [%c%c] not present.\n",
                          required_tags[itype][itag][0], required_tags[itype][itag][1], hline->type[0],
                          hline->type[1]);
                    return 0;
                }
            itag++;
    ...
    while (tags)
        {
            tag = (HeaderTag*)tags->data;
            if (!tag_exists(tag->key, required_tags[itype]) &&
                    !tag_exists(tag->key, optional_tags[itype]))
                {
...
static void
print_header_line(FILE *fp, HeaderLine *hline)
{
    list_t *tags = hline->tags;
    HeaderTag *tag;
...
static void
sam_header_line_free(HeaderLine *hline)
{
    list_t *tags = hline->tags;
...
HeaderDict *
sam_header_clone(const HeaderDict *dict)
{
    HeaderDict *out = NULL;

    while (dict)
        {
            HeaderLine *hline = (HeaderLine*)dict->data;
...
char *
sam_header_write(const void *_header)
{
    int len = 0;
    int nout = 0;
    char *out = NULL;
...
void *
sam_header_parse2(const char *headerText)
{
    int tovalidate = 0;
    size_t nbuf = 0;
    char *buf = NULL;
...
void *
sam_header2tbl(const void *_dict, char type[2], char key_tag[2],
               char value_tag[2])
{
    int ret = 0;
    ...
    // return an empty (not null) hash table
    if (_dict == 0)
        {
            return tbl;
        }
...
char **
sam_header2list(const void *_dict, char type[2], char key_tag[2], int *_n)
{
    int max = 0;
    int n = 0;
    char **ret = NULL;
    ...
    while (l)
        {
            ...
            if (n == max)
                {
                    max = max ? max << 1 : 4;
                    ret = (char**)realloc(ret, (size_t)max * sizeof(char*));
                }
            ret[n++] = key->value;
            l = l->next;
        }
...
void *
sam_header_merge(int n, const void **_dicts)
{
    int idict = 0;
    int status = 0;
            ...
            while (tmpl_hlines)
                {
                    ...
                    while (out_hlines)
                        {
                            status = sam_header_compare_lines((HeaderLine*)tmpl_hlines->data,
                                                              (HeaderLine*)out_hlines->data);
                            if (status == 0)
                                {
                            ...
                            if (status == 2)
                                {
                                    print_header_line(stderr, (HeaderLine*)tmpl_hlines->data);
                                    print_header_line(stderr, (HeaderLine*)out_hlines->data);
                                    debug("Conflicting lines, cannot merge the headers.\n");
                                    return 0;
                                }
                            if (status == 3)
                                {
                                    sam_header_line_merge_with((HeaderLine*)out_hlines->data,
                                                               (HeaderLine*)tmpl_hlines->data);
                                }
                            inserted = 1;
                            break;
                        }
                    if (!inserted)
                        {
                            out_dict = list_append(out_dict,
                                                   sam_header_line_clone((HeaderLine*)tmpl_hlines->data));
                        }
