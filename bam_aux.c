...
void
bam_aux_append(bam1_t *b, const char tag[2], char type, int len, unsigned char *data)
{
...
unsigned char *
bam_aux_get_core(bam1_t *b, const char tag[2])
{
...
unsigned char *
bam_aux_get(const bam1_t *b, const char tag[2])
{
    int y = tag[0] << 8 | tag[1];
    unsigned char *s;
...
int
bam_aux_del(bam1_t *b, unsigned char *s)
{
    unsigned char *p;
    unsigned char *aux;
...
int
bam_aux_drop_other(bam1_t *b, unsigned char *s)
{
    if (s)
        {
            unsigned char *p;
...
void
bam_destroy_header_hash(bam_header_t *header)
{
    if (header->hash)
        {
...
int
bam_aux2i(const unsigned char *s)
{
    int type = 0;
...
float
bam_aux2f(const unsigned char *s)
{
    int type = *s++;
...
double
bam_aux2d(const unsigned char *s)
{
    int type = *s++;
...
char
bam_aux2A(const unsigned char *s)
{
    int type = *s++;
...
char *
bam_aux2Z(const unsigned char *s)
{
    int type = *s++;
