...
void
bam_index_destroy(bam_index_t *idx)
{
    int i = 0;
    khint_t k;
...
bam_index_t *
bam_index_load_local(const char *_fn)
{
    char *fnidx;
    char *fn;
    FILE *fp;
...
static inline int
reg2bins(unsigned int beg, unsigned int end, unsigned short list[BAM_MAX_BIN])
{
    int i = 0;
    int k = 0;
...
static inline int
is_overlap(unsigned int beg, unsigned int end, const bam1_t *b)
{
...
bam_iter_t
bam_iter_query(const bam_index_t *idx, int tid, int beg, int end)
{
    int i = 0;
    int n_bins = 0;
    int n_off = 0;
    unsigned short *bins;
    unsigned long long min_off = 0;
    pair64_t *off;
    khint_t k;
    khash_t(i) *index;
    bam_iter_t iter = 0;
...
void
bam_iter_destroy(bam_iter_t iter)
{
    if (iter)
        {
            free(iter->off);
...
int
bam_iter_read(bamFile fp, bam_iter_t iter, bam1_t *b)
{
    int ret = 0;
