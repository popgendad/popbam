...
template uint64_t* callBase<nucdivData>(nucdivData *t, int n, const bam_pileup1_t *pl);
int makeNucdiv(uint32_t tid, uint32_t pos, int n, const bam_pileup1_t *pl, void *data);
void usageNucdiv(const std::string);

int
mainNucdiv(int argc, char *argv[])
{
...
int
makeNucdiv(uint32_t tid, uint32_t pos, int n, const bam_pileup1_t *pl, void *data)
{
    int i = 0;
    int j = 0;
    int fq = 0;
...
nucdivData::calcNucdiv(void)
{
    ...
    freq = new uint16_t* [npops];
    for (i = 0; i < npops; i++)
        {
            freq[i] = new uint16_t [segsites];
        }
...
int
nucdivData::printNucdiv(const std::string scaffold)
{
    int i = 0;
    int j = 0;
            ...
            for (j = i + 1; j < npops; j++)
                {
                    ...
                    out << '\t' << ns_between[UTIDX(npops,i,j)];
                    if (ns_between[UTIDX(npops,i,j)] >= (uint32_t)((end - beg) * minSites))
                        {
    ...
    try
        {
            ...
            ns_within = new uint64_t [npops] ();
            ns_between = new uint64_t [npops*(npops-1)] ();
...
void
usageNucdiv(const std::string msg)
{
